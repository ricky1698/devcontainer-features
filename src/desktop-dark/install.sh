#!/usr/bin/env bash
#-------------------------------------------------------------------------------------------------------------
# Based on https://github.com/devcontainers/features/tree/main/src/desktop-lite
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See https://go.microsoft.com/fwlink/?linkid=2090316 for license information.
#-------------------------------------------------------------------------------------------------------------
#
# Extended with dark theme support: Fluxbox dark style, GTK Adwaita-dark, dark desktop background.
...
BACKGROUND_COLOR="${BACKGROUNDCOLOR:-#1E1E1E}"
GTK_THEME_NAME="${GTKTHEME:-Adwaita-dark}"
GTK_ICON_THEME="${GTKICONTHEME:-Adwaita}"
...
copy_fluxbox_config() {
    ...
    # Dark theme: set lastwallpaper for dark background
    echo "fbsetroot|-solid ${BACKGROUND_COLOR}|style|:1.0" > "${target_dir}/.fluxbox/lastwallpaper"
}
...
# On older Ubuntu, Tilix is in a PPA. On Debian stretch it's in backports.
...
# Install X11, fluxbox, GTK dark theme packages, and VS Code dependencies
check_packages ${package_list}

# if Ubuntu-24.04, noble(numbat) / Debian-13, trixie found, then install libasound2-dev instead of libasound2.
. /etc/os-release
...
# Replace the default Fluxbox qnx-photon style with dark variant
STYLE_DIR="/usr/share/fluxbox/styles"
if [ -d "$STYLE_DIR" ]; then
    cp "$(dirname "$0")/qnx-photon-dark" "$STYLE_DIR/qnx-photon"
fi

# Configure GTK2 dark theme
USER_HOME="/home/${USERNAME}"
if [ "${USERNAME}" = "root" ]; then
    USER_HOME="/root"
fi

cat > "${USER_HOME}/.gtkrc-2.0" <<GTKEOF
gtk-theme-name="${GTK_THEME_NAME}"
gtk-icon-theme-name="${GTK_ICON_THEME}"
gtk-font-name="Sans 10"
GTKEOF

# Configure GTK3 dark theme
...
# Set GTK_THEME environment variable for all sessions
echo "export GTK_THEME=${GTK_THEME_NAME}" > /etc/profile.d/desktop-dark.sh
chmod +x /etc/profile.d/desktop-dark.sh

##################################
#  Scripts and init files setup  #
##################################
...
# Execute the command if not already running
...
if [ "${USERNAME}" != "root" ]; then
    ...
    chown -R ${USERNAME} /home/${USERNAME}/.Xmodmap /home/${USERNAME}/.fluxbox /home/${USERNAME}/.gtkrc-2.0 /home/${USERNAME}/.config/gtk-3.0
fi
...
You now have a working dark desktop! Connect in one of the following ways:
...
$(if [ -n "${VNC_PASSWORD+x}" ]; then echo "In both cases, use the password \"${VNC_PASSWORD}\" when connecting"; else echo "In both cases, no password is required."; fi)
