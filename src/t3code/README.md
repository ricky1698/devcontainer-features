...
`/tmp/.<16-hex>-00000000.so`, `dlopen()`s it, and **never unlinks it**. So every
...
`T3_TMP_CLEANER_PATTERN` (default `.*-00000000.so` — the stable Bun FFI
temp-file suffix; the 16-hex prefix changes per OpenTUI build, so don't
anchor on it) that are older than
...
ls /tmp/.*-00000000.so 2>/dev/null | wc -l   # current orphan count
```
